# Linear mixed-effects models and dependency  
...
雛の成長度合いは、くちばし長で評価されている。図\@ref(bl-age)は年齢とくちばし長の関連を図示したものである。  
...
それでは、モデル\@ref(qe:lmm)をRで実行する。ここでは、`lme4`パッケージの`lmer`関数を用いる。分析には2012年のデータのみを用いる。      
```{r}
